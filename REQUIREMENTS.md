...
- Sibel gets SMS + email

### Required (Phase 3 — to build)
Manual confirmation flow — Sibel reviews each booking request before it is confirmed.
...
### Step 2 — Sibel's action (3 options)

#### Option A — Accepteren
Sibel clicks "Accepteren" in her email.
...
- Customer receives:
  ...
  - Body: "Beste [naam], uw afspraak bij Q-Atelier is bevestigd. Datum: [datum], Tijd: [tijdstip], Service: [service]. Adres: Laan van Vollenhove 159, 3706 CD Zeist. Tot dan! — Sibel"
- Sibel receives:
...
Sibel clicks "Nieuw tijdstip voorstellen" in her email.

Sibel is taken to a simple webpage (hosted on same S3/CloudFront) where she can:
...
- If customer clicks "Accepteren":
  ...
  - Sibel gets email: "[naam] heeft het nieuwe tijdstip geaccepteerd." + .ics
- If customer clicks "Afwijzen":
  ...
  - Sibel's email includes WhatsApp link: https://api.whatsapp.com/send?phone=[phone]

---

#### Option C — Afwijzen en zelf contact opnemen
Sibel clicks "Afwijzen" in her email.
...
- `suggested_date` (S): new date proposed by Sibel (Option B)
- `suggested_time_slot` (S): new time proposed by Sibel (Option B)
...
- New Lambda endpoints:
  - GET /action?token=xxx&action=accept|reject (Sibel's action from email)
  - GET /reschedule?token=xxx (Sibel's reschedule page)
  - POST /reschedule (Sibel submits new date/time)
  - GET /respond?token=xxx&action=accept|reject (Customer responds to reschedule)
- New S3 pages:
  - /reschedule.html (Sibel's date picker page)
...
DNS validation CNAMEs need to be added but Sibel wants her JouwWeb site to stay live.

### Solution
1. Skip custom domain for now
2. Deploy CloudFront with a self-signed or no custom domain first (test only)
   OR use the raw CloudFront URL (*.cloudfront.net) for Sibel to test
3. Sibel tests and approves the new site on the CloudFront test URL
...
- Sibel can test on that URL
- When ready: re-add domain aliases, validate cert, switch DNS

---

## What Sibel still needs to provide
...
All Sibel-facing content (emails, admin): Dutch (nl)
